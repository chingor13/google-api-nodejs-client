...
export namespace dlp_v2 {
   ...
   * dictionary creation are stored in the specified GCS location. Consider
   * using `CustomInfoType.Dictionary` for smaller dictionaries that satisfy the
   * size requirements.
   */
  export interface Schema$GooglePrivacyDlpV2LargeCustomDictionaryConfig {
    ...
    /**
     * Location to store dictionary artifacts in GCS. These files will only be
     * accessible by project owners and the DLP API. If any of these artifacts
     * are modified, the dictionary is considered invalid and can no longer be
     * used.
     */
  ...
  export interface Schema$GooglePrivacyDlpV2StoredInfoTypeConfig {
    ...
    displayName?: string;
    /**
     * StoredInfoType were findings are defined by a dictionary of phrases.
     */
  ...
  /**
   * CustomInfoType implementation that loads an existing `StoredInfoType`
  ...
  export class Resource$Organizations$Storedinfotypes {
     ...
     * @desc Creates a pre-built StoredInfoType to be used for inspecting
     * storage in InspectDataSource.
     ...
     * @desc Updates the StoredInfoType by creating a new version. The existing
     * version will continue to be used until the new version is ready.
