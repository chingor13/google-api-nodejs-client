...
export namespace digitalassetlinks_v1 {
  ...
  export class Resource$Assetlinks {
     ...
     * @param {string=} params.relation Query string for the relation.  We
     *     identify relations with strings of the format `<kind>/<detail>`,
     *     where `<kind>` must be one of a set of pre-defined purpose
     *     categories, and `<detail>` is a free-form lowercase alphanumeric
     *     string that describes the specific use case of the statement.  Refer
     *     to [our API documentation](/digital-asset-links/v1/relation-strings)
     *     for the current list of supported relations.  For a query to match an
     *     asset link, both the query's and the asset link's relation strings
     *     must match exactly.  Example: A query with relation
     *     `delegate_permission/common.handle_all_urls` matches an asset link
     *     with relation `delegate_permission/common.handle_all_urls`.
     * @param {string=} params.source.androidApp.certificate.sha256Fingerprint
     *     The uppercase SHA-265 fingerprint of the certificate.  From the PEM
     *     certificate, it can be acquired like this:      $ keytool -printcert
     *     -file $CERTFILE | grep SHA256:     SHA256:
     *     14:6D:E9:83:C5:73:06:50:D8:EE:B9:95:2F:34:FC:64:16:A0:83: \
     *     42:E6:1D:BE:A8:8A:04:96:B2:3F:CF:44:E5  or like this:      $ openssl
     *     x509 -in $CERTFILE -noout -fingerprint -sha256     SHA256
     *     Fingerprint=14:6D:E9:83:C5:73:06:50:D8:EE:B9:95:2F:34:FC:64: \
     *     16:A0:83:42:E6:1D:BE:A8:8A:04:96:B2:3F:CF:44:E5  In this example, the
     *     contents of this field would be `14:6D:E9:83:C5:73:
     *     06:50:D8:EE:B9:95:2F:34:FC:64:16:A0:83:42:E6:1D:BE:A8:8A:04:96:B2:3F:CF:
     ...
     * @param {string=} params.source.androidApp.packageName Android App assets
     *     are naturally identified by their Java package name. For example, the
     *     Google Maps app uses the package name `com.google.android.apps.maps`.
     *     REQUIRED
     * @param {string=} params.source.web.site Web assets are identified by a
     *     URL that contains only the scheme, hostname and port parts.  The
     *     format is      http[s]://<hostname>[:<port>]  Hostnames must be fully
     *     qualified: they must end in a single period ("`.`").  Only the
     *     schemes "http" and "https" are currently allowed.  Port numbers are
     *     given as a decimal number, and they must be omitted if the standard
     *     port numbers are used: 80 for http and 443 for https.  We call this
     *     limited URL the "site".  All URLs that share the same scheme,
     ...
     *     `https://google.com/`          (hostname does not match)   *
     *     `https://www.google.com:444/`  (port does not match) REQUIRED
     * @param {string=} params.target.androidApp.certificate.sha256Fingerprint
     *     The uppercase SHA-265 fingerprint of the certificate.  From the PEM
     *     certificate, it can be acquired like this:      $ keytool -printcert
     *     -file $CERTFILE | grep SHA256:     SHA256:
     *     14:6D:E9:83:C5:73:06:50:D8:EE:B9:95:2F:34:FC:64:16:A0:83: \
     ...
     *     string and represent the result as a hexstring (that is, uppercase
     *     hexadecimal representations of each octet, separated by colons).
     * @param {string=} params.target.androidApp.packageName Android App assets
     *     are naturally identified by their Java package name. For example, the
     *     Google Maps app uses the package name `com.google.android.apps.maps`.
     *     REQUIRED
     * @param {string=} params.target.web.site Web assets are identified by a
     *     URL that contains only the scheme, hostname and port parts.  The
     *     format is      http[s]://<hostname>[:<port>]  Hostnames must be fully
     *     qualified: they must end in a single period ("`.`").  Only the
     *     schemes "http" and "https" are currently allowed.  Port numbers are
  ...
  export class Resource$Statements {
     ...
     *
     * @param {object} params Parameters for request
     * @param {string=} params.relation Use only associations that match the
     *     specified relation.  See the [`Statement`](#Statement) message for a
     *     detailed definition of relation strings.  For a query to match a
     *     statement, one of the following must be true:  *    both the query's
     *     and the statement's relation strings match exactly,      or *    the
